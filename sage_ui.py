...
def open_app(command):
    """Open applications based on command with improved error handling"""
    app_mapping = {
        "excel": ("Excel", "excel.exe", r"C:\Program Files\Microsoft Office\root\Office16\EXCEL.EXE"),
        "powerpoint": ("PowerPoint", "powerpnt.exe", r"C:\Program Files\Microsoft Office\root\Office16\POWERPNT.EXE"),
        "word": ("Word", "winword.exe", r"C:\Program Files\Microsoft Office\root\Office16\WINWORD.EXE"),
    ...
    for app, app_info in app_mapping.items():
        if app in command:
            ...
            try:
                ...
                if system == "Windows":
                    # Try specific path for Office apps if available
                    if len(app_info) > 2 and "office" in display_name.lower():
                        try:
                            subprocess.Popen(app_info[2])
                            say(f"Opening {display_name}.")
                            return True
                        except Exception as e:
                            print(f"Specific path failed, trying generic: {e}")

                    # Fallback to generic method
                    if app == "camera":
                        os.system(app_info[1])
                    elif app == "google":
                ...
                say(f"Could not open {display_name}. Please make sure it's installed.")
                return False
...
def send_email(subject, body, to_email):
    ...
    from_email = "gppg317@gmail.com" # use your username 
    password = "AGboy@@2001" # use your password
...
def ai(prompt):
    ...
    try:
        openai.api_key = "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" #your groq api key
...
def check_unread_emails():
    """Check and summarize unread emails with better handling"""
    import imaplib
    import email
    from email.header import decode_header
    import re

    username = "gppg317@gmail.com"
    password = "AGboy@@2001"  # Consider using environment variables

    say("Checking your unread emails now...")
    ...
    try:
        ...
        if status != "OK":
            say("Sorry, I couldn't check your emails right now.")
            return
        ...
        say(f"You have {len(email_ids)} unread email{'s' if len(email_ids) > 1 else ''}.")

        # Process up to 5 most recent emails
        summaries = []
        for i, email_id in enumerate(email_ids[:5], 1):
            ...
            for response_part in msg_data:
                if isinstance(response_part, tuple):
                    ...
                    # Build summary line
                    summary = f"Email {i}: From {sender_name}, Subject: {subject}"
                    if date:
                        summary += f" (Received: {date})"
                    ...
                    if body:
                        clean_body = ' '.join(body.split()[:20])  # First 20 words
                        summaries.append(f"Preview: {clean_body}...")

        # Read the summaries
        if summaries:
            say("Here's your email summary:")
            for line in summaries:
                say(line)
        else:
            say("I couldn't retrieve the email contents.")

    except Exception as e:
        print(f"Email error: {e}")
        say("Sorry, I encountered an error while checking your emails.")
    finally:
        try:
            mail.close()
            mail.logout()
...
def introduce_assistant():
    """Introduce the assistant with comprehensive information"""
    intro = """
    Hello! I'm SAGE, your advanced AI voice assistant. 
...
def main():
    global using_terminator, searching, expecting_code, conversation_history, first_run
    if first_run:
        introduce_assistant()
        first_run = False
    if listen_btn:
        ...
        if any(cmd in query.lower() for cmd in ["open ", "start "]):
            if "google" in query.lower() or "chrome" in query.lower():
                if "search" in query.lower():
                    search_term = query.lower().replace("open", "").replace("google", "").replace("chrome", "").replace("search", "").strip()
                    if search_term:
                        search_query = quote(search_term)
                        url = f"https://www.google.com/search?q={search_query}"
                        webbrowser.open(url)
